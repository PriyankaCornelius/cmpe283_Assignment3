...
extern atomic64_t total_exit_processing_time;
extern atomic64_t array_exit_count[69];
...
static int __vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	...
	if(exit_reason.basic <69){
		atomic64_inc(&array_exit_count[exit_reason.basic]);
	}
	...
	atomic64_add(end_time, &total_exit_processing_time);
	
	printk("CPUID(0x4FFFFFFF),Exit Count : %d, Cycles spent in exit : %llu", atomic_read(&exit_count), atomic64_read(&total_exit_processing_time));
