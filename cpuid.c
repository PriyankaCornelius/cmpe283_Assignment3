...
atomic64_t total_exit_processing_time;
EXPORT_SYMBOL(total_exit_processing_time);
atomic64_t array_exit_count[69];
EXPORT_SYMBOL(array_exit_count);
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if (eax == 0x4FFFFFFF) {
	    u64 exit_time = atomic64_read(&total_exit_processing_time);
		...
		ecx = low_32_bits;
		edx = 0;
		printk(KERN_INFO "Exit time : %llu", exit_time);
	}
	if (eax == 0x4FFFFFFE) {
		if (ecx < 0 || ecx == 35 || ecx == 38 || ecx == 42 || ecx == 65 || ecx > 68){
			printk(KERN_INFO "Exit reason : %u not defined in SDM", ecx);
			eax = 0;	
			ebx = 0;
			ecx = 0;
			edx = 0xFFFFFFFF;
			...
			eax = atomic64_read(&array_exit_count[ecx]);
			printk("CPUID(0x4FFFFFFE), Exit number: %d, Exits : %llu", ecx, atomic64_read(&array_exit_count[ecx]));
		}
	}
	else {
